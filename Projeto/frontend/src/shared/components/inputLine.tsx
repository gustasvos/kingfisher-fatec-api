...
export default function InputLine({ classNameInput, classNameLabel, id, htmlfor, children, type, name, value, required, checked,onchange }: PropriedadeInput) {
    ...
    return (
        <section className="relative">
            <input type={type} name={name} id={id} required={required} onChange={onchange} className={`w-[300px] block rounded-t-lg px-2.5 pb-2.5 pt-5 text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer ${classNameInput}`} />
